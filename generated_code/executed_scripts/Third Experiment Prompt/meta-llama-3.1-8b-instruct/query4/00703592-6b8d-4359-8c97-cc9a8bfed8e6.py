        ...
        for filename in filenames:
            ...
            if filename.endswith('.json'):
                ...
                with open(os.path.join(dirpath, filename), 'r') as f:
                    # Load the JSON data
                    data = json.load(f)
                    # Check if the JSON data has a 'monetization' key
                    if 'monetization' in data:
                        # Iterate over the monetization data
                        for item in data['monetization']['eligibility']:
                            # Extract the company name from the item
                            company_name = item['string_map_data']['Product Name']['value']
