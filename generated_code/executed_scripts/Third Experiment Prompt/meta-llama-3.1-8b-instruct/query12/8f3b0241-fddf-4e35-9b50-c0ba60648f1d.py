        ...
        if 'inbox' in dirnames:
            ...
            for filename in os.listdir(inbox_dir):
                ...
                if filename.startswith('message_') and filename.endswith('.json'):
                    ...
                    file_week = datetime.datetime.strptime(filename, 'message_%d.json').strftime('%Y-%W')

                    # Check if the week is different from the previous week
                    if week is None or file_week != week:
                        # Update the week and reset the message count
                        week = file_week
                        message_count = 0
