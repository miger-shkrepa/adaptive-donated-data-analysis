...
if not os.path.exists(login_activity_path):
    ...
    with open(output_csv_path, mode='w', newline='') as csv_file:
        ...
        for entry in login_data["account_history_login_history"]:
            device_id = entry["string_map_data"].get("GerÃ¤te-ID", {}).get("value", "")
            login_time = entry["string_map_data"].get("Zeit", {}).get("timestamp", 0)
            if login_time:
                login_time_str = timestamp_to_datetime(login_time)
                csv_writer.writerow([device_id, login_time_str])
            else:
                csv_writer.writerow([device_id, ""])
