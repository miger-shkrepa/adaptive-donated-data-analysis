...
def extract_device_and_login_time(data):
    ...
    if "account_history_login_history" in data:
        for entry in data["account_history_login_history"]:
            device_id = entry["string_map_data"].get("User Agent", {}).get("value", "")
            timestamp = entry["string_map_data"].get("Zeit", {}).get("timestamp", 0)
            if timestamp:
...
def main():
    try:
        login_activity_file_path = get_login_activity_file_path(root_dir)
        data = read_json_file(login_activity_file_path)
