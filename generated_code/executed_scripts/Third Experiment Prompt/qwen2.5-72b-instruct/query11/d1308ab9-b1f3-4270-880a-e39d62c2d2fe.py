    ...
    try:
        ...
        for entry in data.get("impressions_history_posts_seen", []):
            for string_map in entry.get("string_map_data", []):
                if "Author" in string_map:
                    viewed_accounts.add(string_map["Author"]["value"])
...
def find_accounts_viewed_not_liked(root_dir):
    ...
    if os.path.exists(posts_viewed_file):
        viewed_accounts = get_viewed_accounts(posts_viewed_file)
    if os.path.exists(liked_posts_file):
        liked_accounts = get_liked_accounts(liked_posts_file)
