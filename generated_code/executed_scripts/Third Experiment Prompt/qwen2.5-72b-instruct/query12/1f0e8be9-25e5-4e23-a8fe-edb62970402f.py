...
def get_all_messages(root_dir):
    ...
    for conversation_dir in os.listdir(inbox_dir):
        ...
        if os.path.isdir(conversation_path):
            messages = get_messages_from_conversation(conversation_path)
            conversation_weekly_counts = process_messages(messages)
            for week, count in conversation_weekly_counts.items():
...
def main():
    ...
    try:
        weekly_counts = get_all_messages(root_dir)
