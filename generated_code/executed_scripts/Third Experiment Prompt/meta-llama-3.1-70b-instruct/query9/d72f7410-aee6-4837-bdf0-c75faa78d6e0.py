...
def get_followers_profiles(root_dir):
    followers_file_path = os.path.join(root_dir, "connections", "followers_and_following", "followers.json")
    try:
        with open(followers_file_path, 'r') as followers_file:
            followers_data = json.load(followers_file)
            followers_profiles = [profile["title"] for profile in followers_data["relationships_followers"]]
