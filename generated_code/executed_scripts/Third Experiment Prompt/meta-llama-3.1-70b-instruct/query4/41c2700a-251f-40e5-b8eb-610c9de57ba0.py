...
def get_companies_with_access(root_dir):
    ...
    try:
        ...
        if not os.path.exists(ads_info_dir):
            raise FileNotFoundError("Error: The 'ads_information/instagram_ads_and_businesses' directory does not exist.")
        
        advertisers_file = os.path.join(ads_info_dir, "advertisers_using_your_activity_or_information.json")
