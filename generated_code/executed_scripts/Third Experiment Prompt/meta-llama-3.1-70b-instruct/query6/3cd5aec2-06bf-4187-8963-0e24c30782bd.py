...
def get_account_changes(root_dir):
    account_changes = []
    account_information_file = os.path.join(root_dir, "personal_information", "personal_information", "account_information.json")
    
    if not os.path.exists(account_information_file):
        raise FileNotFoundError("FileNotFoundError: The account information file does not exist.")
    
    with open(account_information_file, 'r') as f:
        data = json.load(f)
        
        for change in data["profile_account_insights"]:
            for key, value in change["string_map_data"].items():
                if key in ["Name", "E-Mail-Adresse", "Telefonnummer bestätigt"]:
