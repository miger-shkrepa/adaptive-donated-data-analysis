...
def check_directory_structure(root_dir):
    try:
        ...
        if not os.path.exists(os.path.join(root_dir, "ads_information")):
            return False
        if not os.path.exists(os.path.join(root_dir, "ads_information", "ads_and_topics")):
            return False
        if not os.path.exists(os.path.join(root_dir, "ads_information", "ads_and_topics", "ads_viewed.json")):
            return False
        return True
    except Exception as e:
        raise ValueError("Error: " + str(e))
...
def main(root_dir):
    try:
        if not check_directory_structure(root_dir):
            with open("query_responses/results.csv", "w", newline="") as file:
                writer = csv.writer(file)
                writer.writerow(["Company Name", "Number of Ads Viewed"])
            return
