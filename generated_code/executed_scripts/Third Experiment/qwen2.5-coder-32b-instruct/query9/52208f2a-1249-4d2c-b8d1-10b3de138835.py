        ...
        with open(following_file_path, 'r') as following_file:
            ...
            for item in following_data['relationships_following']:
                for string_data in item['string_list_data']:
                    following_profiles.add(string_data['value'])
    ...
    try:
        with open(followers_file_path, 'r') as followers_file:
            ...
            for item in followers_data:
                for string_data in item['string_list_data']:
                    followers_profiles.add(string_data['value'])
    except Exception as e:
        raise ValueError(f"ValueError: Error reading followers_1.json - {str(e)}")

    non_mutual_profiles = following_profiles - followers_profiles

    return non_mutual_profiles
...
try:
    non_mutual_profiles = get_following_profiles(root_dir)
    save_to_csv(non_mutual_profiles, 'query_responses/results.csv')
except Exception as e:
    print(e)
