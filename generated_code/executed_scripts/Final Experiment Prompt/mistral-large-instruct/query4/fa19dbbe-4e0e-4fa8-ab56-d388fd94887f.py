...
def extract_company_names(json_file_path):
    try:
        ...
        raise ValueError("ValueError: The expected key 'ig_custom_audiences_all_types' is missing in the JSON data.")
    except Exception as e:
        raise ValueError(f"ValueError: An unexpected error occurred: {e}")

def save_to_csv(company_names, output_csv_path):
    try:
        ...
        raise ValueError(f"ValueError: An error occurred while writing to the CSV file: {e}")

def main():
    company_names = extract_company_names(json_file_path)
    save_to_csv(company_names, output_csv_path)
