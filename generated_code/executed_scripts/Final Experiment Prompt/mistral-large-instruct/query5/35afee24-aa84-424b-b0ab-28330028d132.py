...
def process_devices_file(file_path):
    ...
    try:
        ...
        raise ValueError(f"Error: Failed to read or parse the JSON file - {e}")
    ...
    for device in devices_data:
        ...
        if user_agent and last_login_timestamp is not None:
            device_id = user_agent  # Assuming 'User Agent' value is the 'Device ID'
...
def write_results_to_csv(results, output_path):
    try:
        ...
        raise ValueError(f"Error: Failed to write the CSV file - {e}")
...
def main():
    try:
        ...
        print(e)
        # If an error occurs, write only the headers to the CSV file
