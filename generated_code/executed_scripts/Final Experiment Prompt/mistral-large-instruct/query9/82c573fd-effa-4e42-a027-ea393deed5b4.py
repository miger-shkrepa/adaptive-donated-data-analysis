...
def main():
    ...
    try:
        ...
        print(e)
        # Create a CSV file with only the column headers if files are missing
        with open('query_responses/results.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["Profile"])
        return

    following_profiles = extract_profiles(following_data.get("relationships_following", []), "relationships_following")
