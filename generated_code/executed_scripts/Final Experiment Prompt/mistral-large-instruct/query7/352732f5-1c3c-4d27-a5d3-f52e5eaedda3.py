    ...
    for filename in os.listdir(story_interactions_dir):
        if filename.endswith(".json"):
            ...
            try:
                ...
                raise ValueError(f"ValueError: Failed to decode JSON from file {file_path}.")
            except Exception as e:
                raise RuntimeError(f"RuntimeError: An error occurred while processing file {file_path}. {str(e)}")
...
def save_to_csv(user_engagement, output_path):
    try:
        ...
        raise RuntimeError(f"RuntimeError: An error occurred while writing to CSV. {str(e)}")
