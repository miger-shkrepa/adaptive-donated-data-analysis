...
with open(output_file_path, 'w', newline='') as csvfile:
    ...
    for post in data['impressions_history_posts_seen']:
        ...
        writer.writerow({
            'Date/Week': date.strftime('%Y-%m-%d') if date.strftime('%Y-%m-%d') == date.strftime('%Y-%m-%d') else f"Week {week}",
            'Posts Viewed': 1,
            'Type': 'Daily' if date.strftime('%Y-%m-%d') == date.strftime('%Y-%m-%d') else 'Weekly'
        })
