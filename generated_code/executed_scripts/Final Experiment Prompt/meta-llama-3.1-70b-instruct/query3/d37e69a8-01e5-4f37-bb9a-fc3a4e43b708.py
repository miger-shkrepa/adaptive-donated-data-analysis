...
def get_post_views(root_dir):
    ...
    Returns:
    A dictionary with daily and weekly post views.
    """
    post_views = {}
        ...
        with open(file_path, "r") as file:
            ...
            for post in data["impressions_history_posts_seen"]:
                ...
                if date_str not in post_views:
                    post_views[date_str] = {"Daily": 1, "Weekly": 0}
                else:
                    post_views[date_str]["Daily"] += 1
                
                # Update weekly post views
                if week_str not in post_views:
                    post_views[week_str] = {"Daily": 0, "Weekly": 1}
                else:
                    post_views[week_str]["Weekly"] += 1
    except json.JSONDecodeError:
        raise ValueError("Error: Invalid JSON format in the required file.")
    except KeyError:
        raise ValueError("Error: Invalid JSON structure in the required file.")
...
def save_to_csv(post_views):
    ...
    Args:
    post_views (dict): A dictionary with daily and weekly post views.
    """
