...
def get_user_engagement(root_dir):
    ...
    # Iterate over each file in the story interactions directory
                        ...
                        for section in story_interaction_sections:
                            if section in data:
                                for item in data[section]:
                                    user = item["title"]
                                    engagement_count = len(item.get("string_list_data", []))
                                    if user in user_engagement:
                                        user_engagement[user] += engagement_count
                                    else:
                                        user_engagement[user] = engagement_count
                except json.JSONDecodeError:
                    raise ValueError("Error: The JSON file is not valid.")
    else:
        # If the story interactions directory does not exist, return an empty dictionary
        pass
...
def save_to_csv(user_engagement):
    """
    This function saves the user engagement data to a CSV file.
    ...
    # Save the user engagement data to the CSV file
    with open(csv_file_path, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["User", "Times Engaged"])
        for user, engagement_count in user_engagement.items():
