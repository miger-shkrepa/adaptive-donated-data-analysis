...
def save_to_csv(device_login_info):
    ...
    try:
        ...
        raise ValueError(f"Error: Failed to save the device login information to a CSV file. {str(e)}")

def main():
    device_login_info = get_device_login_info(root_dir)
    if not device_login_info:
        # If no device login information is found, save a CSV file with only the column headers
