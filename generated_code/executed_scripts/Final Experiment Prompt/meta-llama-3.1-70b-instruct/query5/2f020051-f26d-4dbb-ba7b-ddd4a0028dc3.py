...
def get_device_login_info(root_dir):
    ...
    try:
        ...
        # Load the JSON data from the devices.json file
        ...
        for device in data.get("devices_devices", []):
            ...
            # Extract the device ID from the user agent string
            device_id = user_agent.get("value", "")

            # Extract the login time from the last login timestamp
            login_time = last_login.get("timestamp", 0)
            if login_time > 0:
                login_time = datetime.utcfromtimestamp(login_time).strftime("%Y-%m-%d %H:%M:%S")
            else:
                login_time = ""
        ...
        raise ValueError("Error: Failed to parse the JSON data.")
