...
def count_interactions(data, key):
    ...
    if key == "Media Owner":
        for entry in data:
            value = entry.get("string_map_data", {}).get(key, {}).get("value", "")
...
def main():
    try:
        ...
        # Count interactions
        post_likes = count_interactions(liked_posts_data.get("likes_media_likes", []) if liked_posts_data else [], "title")
        story_likes = count_interactions(story_likes_data.get("story_activities_story_likes", []) if story_likes_data else [], "title")
        comments = count_interactions(reels_comments_data.get("comments_reels_comments", []) if reels_comments_data else [], "Media Owner")
