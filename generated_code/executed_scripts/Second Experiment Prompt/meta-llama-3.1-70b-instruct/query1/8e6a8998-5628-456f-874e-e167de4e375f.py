...
def get_topics_of_interest(root_dir):
    try:
        ...
        if os.path.exists(locations_of_interest_path):
            # Open the 'locations_of_interest.json' file and read its content
            with open(locations_of_interest_path, 'r') as file:
                # Since the exact structure of the 'locations_of_interest.json' file is not provided,
        ...
        if os.path.exists(recommended_topics_path):
            # Open the 'recommended_topics.json' file and read its content
            with open(recommended_topics_path, 'r') as file:
                # Since the exact structure of the 'recommended_topics.json' file is not provided,
                # we assume it contains a list of topics of interest
                import json
                data = json.load(file)
...
def main():
    try:
        ...
        # If an error occurs, write an empty CSV file
