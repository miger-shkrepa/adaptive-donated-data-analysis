...
def process_json_file(file_path):
    try:
        with open(file_path, 'r') as file:
            ...
            if "likes_media_likes" in json_data["structure"]:
                for item in json_data["structure"]["likes_media_likes"]:
                    result["User"].append(item["title"])
                    result["Post Likes"].append(len(item["string_list_data"]))
            if "story_activities_story_likes" in json_data["structure"]:
                for item in json_data["structure"]["story_activities_story_likes"]:
                    result["User"].append(item["title"])
                    result["Story Likes"].append(len(item["string_list_data"]))
            if "comments_reels_comments" in json_data["structure"]:
                for item in json_data["structure"]["comments_reels_comments"]:
                    result["User"].append(item["sender_name"])
