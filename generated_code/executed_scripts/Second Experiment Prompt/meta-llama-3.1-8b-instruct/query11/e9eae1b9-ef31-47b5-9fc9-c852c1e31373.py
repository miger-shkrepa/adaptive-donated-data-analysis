...
for dir_name in os.listdir(root_dir):
    ...
    if os.path.isdir(dir_path):
        ...
        if os.path.exists(likes_file) and os.path.exists(saved_file):
            ...
            account_names = set()
            if "structure" in likes_data and "likes_media_likes" in likes_data["structure"]:
            ...
            if "structure" in saved_data and "saved_saved_media" in saved_data["structure"]:
                for post in saved_data["structure"]["saved_saved_media"]:
                    account_names.add(post["title"])

            # Find the accounts that have viewed posts but not liked them
            viewed_but_not_liked = []
            if "structure" in likes_data and "likes_media_likes" in likes_data["structure"]:
                for post in likes_data["structure"]["likes_media_likes"]:
                    if post["title"] not in account_names:
                        viewed_but_not_liked.append(post["title"])
            else:
                viewed_but_not_liked = list(account_names)
