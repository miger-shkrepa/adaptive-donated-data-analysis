...
def write_to_csv(post_views, video_views):
    ...
    with open(output_path, 'w', newline='') as csvfile:
        ...
        # Count post views
        for author, _ in post_views:
            if author not in view_counts:
                view_counts[author] = {'Post Views': 0, 'Video Views': 0}
            view_counts[author]['Post Views'] += 1

        # Count video views
        for author, _ in video_views:
            if author not in view_counts:
                view_counts[author] = {'Post Views': 0, 'Video Views': 0}
            view_counts[author]['Video Views'] += 1

        # Write the counts to the CSV file
        for author, counts in view_counts.items():
            writer.writerow({'Account': author, 'Post Views': counts['Post Views'], 'Video Views': counts['Video Views']})
