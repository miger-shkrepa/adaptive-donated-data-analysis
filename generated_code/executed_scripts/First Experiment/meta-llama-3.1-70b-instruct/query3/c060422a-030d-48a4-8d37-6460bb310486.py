...
def get_post_views(root_dir):
    try:
        ...
        for date, views in post_views.items():
            ...
            if 'Daily' in views and views['Daily'] > 0:
                csv_data.append([date, views['Daily'], 'Daily'])
            # Check if the date is a weekly date
            if 'Weekly' in views and views['Weekly'] > 0:
                csv_data.append([date, views['Weekly'], 'Weekly'])
