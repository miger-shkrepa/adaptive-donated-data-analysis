            ...
            for filename in os.listdir(story_sticker_interactions_dir):
                if filename.endswith(".json"):
                    ...
                    with open(file_path, "r") as file:
                        data = json.load(file)
                        for activity in data.get("story_activities_emoji_sliders", []) + data.get("story_activities_polls", []) + data.get("story_activities_questions", []) + data.get("story_activities_quizzes", []) + data.get("story_activities_story_likes", []):
