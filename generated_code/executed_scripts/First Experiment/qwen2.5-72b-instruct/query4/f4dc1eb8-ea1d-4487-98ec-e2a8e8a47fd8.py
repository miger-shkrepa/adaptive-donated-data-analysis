                            ...
                            if 'your_instagram_activity' in dirpath:
                                for key, value in data.items():
                                    if isinstance(value, list):
                                        for item in value:
                                            if 'string_map_data' in item and 'User Agent' in item['string_map_data']:
                                                user_agent = item['string_map_data']['User Agent']['value']
                                                if 'company' in user_agent.lower():
                                                    companies_with_access.add(user_agent.split(' ')[0])
