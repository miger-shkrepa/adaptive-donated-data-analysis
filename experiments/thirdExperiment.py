"""
Third Experiment of Master Thesis.

:author: Miger Shkrepa
"""

from src import helper, evaluations, dynamic_get_directory_structure
        ...
        for model in models:
            ...
            for dataset_folder in selected_datasets:
                if model == "meta-llama-3.1-8b-instruct":
                    ...
                    dynamic_get_directory_structure.generate_structure(f"datasets/{dataset_name}",
                                                                       "../data/directory_structure_final.txt")
                    ...
                    base_prompt = (
                            ...
                            "10) The directory structure provided with this request accurately reflects the complete contents of the current data package. However, each data package may focus on a specific subset of user data (e.g., only activity or everything except ads), and may therefore lack information relevant to certain queries. As such, you must evaluate whether the available structure contains sufficient data to answer the question. If the necessary files or directories do not exist in the structure, and they are required to answer the question, return a CSV file with only the column headers. If a missing file contributes to only part of the aggregation or calculation(e.g., one of multiple columns), treat its contribution as 0 and continue processing the rest using the available data.\n"
                    )
                    ...
                    while error_count <= max_attempts:
                        ...
                        if not result["error"]:
                            ...
                            os.makedirs(f"generated_code/executed_scripts/Third Experiment/{model}/{query_dir_name}", exist_ok=True)
                            with open(f"generated_code/executed_scripts/Third Experiment/{model}/{query_dir_name}/{experiment_id}.py", "w",
                                      encoding="utf-8") as f:
